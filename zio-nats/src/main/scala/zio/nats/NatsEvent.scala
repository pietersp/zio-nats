 ...
 *   - `Resubscribed`        — subscriptions were re-established after a reconnect. Distinct
 *                             from `Reconnected`: `Reconnected` fires when the TCP connection
 *                             is restored, `Resubscribed` fires when all SUB commands have
 *                             been re-sent to the server.
 *   - `ServersDiscovered`   — the server advertised new cluster members (signal only; jnats 2.x
 *                             does not expose the discovered URLs in the listener callback).
