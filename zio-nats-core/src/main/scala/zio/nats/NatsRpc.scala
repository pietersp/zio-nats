...
object NatsRpc {
   ...
   *
   * Messages without a `replyTo` subject are silently ignored. Per-message
   * failures (decode errors, handler errors, encode errors) are logged via
   * ZIO's default fiber-failure logger and skipped — the responder fiber
   * continues processing subsequent messages. Only infrastructure-level stream
   * errors (e.g. connection loss) terminate the responder fiber.
   *
    ...
    for {
      ...
      _    <- nats
             ...
             .mapZIO { rawEnv =>
               (for {
                 decoded <- ZIO
                              .fromEither(rawEnv.message.decode[A])
                 ...
                 _        <- rawEnv.message.replyTo match {
                        case Some(replyTo) =>
                          ZIO
                            ...
                            .mapError(e =>
                              NatsError.SerializationError(s"Failed to encode RPC response: ${e.toString}", e)
                            )
