...
| `noEcho`                               | `Boolean`      | `false`                         | Suppress messages the connection published      |
...
| `reconnectJitter`                      | `Duration`     | `100.millis`                    | Random jitter added to reconnect wait           |
...
|----------------------|-------------------------------------------------------------------------------|
| `servers`            | `NATS_SERVERS` (comma-separated for multiple: `nats://a:4222,nats://b:4222`) |
...
| `connection-timeout` | `NATS_CONNECTION_TIMEOUT` (ISO-8601: `PT5S` = 5s, `PT2M` = 2min)            |
