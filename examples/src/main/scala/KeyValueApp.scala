 ...
 *
 * NATS KV is built on JetStream and stores typed values under string keys with
 * revisioned history and change-watch streams. This example covers:
 *
 *   1. put / get — basic typed read and write
 *   2. create / update — conditional writes: create-if-absent and
 *      compare-and-swap by revision
 *   3. watch — live stream of KvEvents; emits Put, Delete, and Purge events as
 *      keys change
